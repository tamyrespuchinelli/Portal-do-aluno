...
$query = "SELECT disciplina.codisc,disc_nome,classe.classe_id from disciplina inner join classe on disciplina.codisc=classe.codisc inner join cursa on classe.classe_id=cursa.classe_id where a_matricula='".$aluno."'";
$inserir = mysqli_query($conexao, $query);
while($rowData = mysqli_fetch_array($inserir))
	echo '<a href="material/'.$rowData['codisc'].'"><button class="button_turma">'.$rowData['disc_nome'].'</button></a>';
//echo '<a href="material/'.$rowData['codisc'].'">Material</a><input type="text"  value="'.$rowData['codisc'].'" size="5">'
